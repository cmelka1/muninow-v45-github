...
import { useBusinessLicensesMerchants } from '@/hooks/useBusinessLicensesMerchants';
...
interface BusinessLicensesMunicipalityAutocompleteProps {
  value?: string;
  onSelect: (merchant: BusinessLicensesMerchant) => void;
  placeholder?: string;
  disabled?: boolean;
  className?: string;
}

export const BusinessLicensesMunicipalityAutocomplete: React.FC<BusinessLicensesMunicipalityAutocompleteProps> = ({
  value,
  onSelect,
  ...
  const [selectedMerchant, setSelectedMerchant] = useState<BusinessLicensesMerchant | null>(null);
  
  const inputRef = useRef<HTMLInputElement>(null);
  const dropdownRef = useRef<HTMLDivElement>(null);
  
  const { merchants, isLoading, error } = useBusinessLicensesMerchants(searchTerm);
  ...
  const getDisplayName = (merchant: BusinessLicensesMerchant) => {
  ...
  const handleSelect = (merchant: BusinessLicensesMerchant) => {
