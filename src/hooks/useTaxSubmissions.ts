...
interface UseTaxSubmissionsParams extends PaginationParams {
  filters?: TaxSubmissionFilters;
}

export const useTaxSubmissions = (params?: UseTaxSubmissionsParams) => {
  const { user } = useAuth();
  const { page = 1, pageSize = 5, filters = {} } = params || {};

  return useQuery({
    queryKey: ['tax-submissions', user?.id, page, pageSize, filters],
    queryFn: async () => {
