...
export const useBusinessLicensePaymentMethods = (license: any) => {
  ...
  const handlePayment = async (): Promise<PaymentResponse> => {
    ...
    try {
      ...
      if (data.success) {
        ...
        // Refresh the page to show updated license status
  ...
  const handleGooglePayment = async (): Promise<PaymentResponse> => {
    try {
      ...
      const { data, error } = await supabase.functions.invoke('process-business-license-google-pay-payment', {
        body: {
          license_id: license.id,
  ...
  const handleApplePayment = async (): Promise<PaymentResponse> => {
    try {
      ...
      const onPaymentAuthorized = async (event: any) => {
        ...
        const { data, error } = await supabase.functions.invoke('process-business-license-apple-pay-payment', {
          body: {
            license_id: license.id,
