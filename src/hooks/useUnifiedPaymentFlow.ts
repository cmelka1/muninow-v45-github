...
export const useUnifiedPaymentFlow = (params: UnifiedPaymentFlowParams) => {
  ...
  const handleGooglePayment = async (): Promise<PaymentResponse> => {
    ...
    try {
      ...
      // Initialize Google Pay client - use explicit env var only (not PROD)
      ...
      console.log('🌍 useUnifiedPaymentFlow Google Pay environment:', isProduction ? 'PRODUCTION' : 'TEST');
  ...
  // Apple Pay removed - now handled by useApplePayFlow hook
