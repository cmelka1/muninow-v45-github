...
interface ProcessPermitGooglePayTransferRequest {
  permit_id: string;
...
serve(async (req) => {
  ...
  try {
    ...
    // Parse request body
    const body: ProcessPermitGooglePayTransferRequest = await req.json();
    const { 
      permit_id, 
      google_pay_token, 
      total_amount_cents, 
    ...
    console.log("Processing Google Pay permit payment:", { 
      permit_id, 
      total_amount_cents, 
      user_id: user.id,
    ...
    // Validate input
    if (!permit_id || !google_pay_token || !total_amount_cents || !idempotency_id) {
    ...
    if (transferResponse.ok && transferData.state === 'SUCCEEDED') {
      try {
        ...
        console.log("Permit updated successfully for Google Pay payment:", transferData.id);
      } catch (error) {
    ...
    console.error("Google Pay permit payment error:", error);
