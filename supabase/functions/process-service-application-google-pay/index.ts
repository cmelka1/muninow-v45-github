...
interface ProcessServiceApplicationGooglePayRequest {
...
serve(async (req) => {
  ...
  try {
    ...
    // Parse request body
    const body: ProcessServiceApplicationGooglePayRequest = await req.json();
    const { 
      application_id, 
      google_pay_token, 
      billing_contact, 
    ...
    const isCard = true; // Google Pay is always card-based
    ...
    // Create Finix Payment Instrument from Google Pay token
    const paymentInstrumentPayload = {
      ...
      ...(billing_contact && {
        billing_address: {
          ...
          country: billing_contact.country_code || "USA",
          line1: billing_contact.address1,
          line2: billing_contact.address2,
          city: billing_contact.locality,
          region: billing_contact.administrative_area,
        }
      })
    };
    ...
    if (!paymentInstrumentResponse.ok) {
      ...
      return new Response(
        JSON.stringify({ 
          success: false, 
          error: 'Failed to process Google Pay token'
        }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
    ...
    console.log('Creating Finix Transfer for Google Pay payment');
    ...
    console.log('Google Pay service application payment completed successfully');
    ...
    console.error('Service application Google Pay payment error:', error);
    return new Response(
      JSON.stringify({ 
        success: false, 
