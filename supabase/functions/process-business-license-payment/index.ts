...
serve(async (req) => {
  ...
  try {
    ...
    // Get business license details and validate ownership
    ...
    if (licenseError || !license) {
      throw new Error("Business license not found, access denied, or not approved for payment");
    }
    ...
    // Create payment history record with license details
    const { data: paymentHistory, error: phError } = await supabaseService
      ...
      .insert({
        ...
        doing_business_as: license.merchant_name,
        statement_descriptor: license.merchant_name,
    ...
    // If transfer succeeded, update the business license
    if (finixResponse.ok && finixData.state === 'SUCCEEDED') {
      try {
        const { error: licenseUpdateError } = await supabaseService
          .from("business_license_applications")
          .update({
            service_fee_cents: calculatedServiceFee,
            total_amount_cents: total_amount_cents,
        ...
        if (licenseUpdateError) {
          console.error("Error updating business license:", licenseUpdateError);
          throw new Error("Failed to update business license status");
        }

        console.log("Business license updated successfully for payment:", finixData.id);
      } catch (error) {
        console.error("Business license update failed:", error);
        // Note: Payment was successful, but license update failed
    ...
    console.error("Business license payment error:", error);
