-- Fix incorrect join in can_view_profile_for_permits function
-- The permit_applications table uses permit_id as primary key, not id
...
BEGIN
  ...
  IF target_user_customer_id IS NOT NULL THEN
    ...
    -- FIXED: Changed pa.id to pa.permit_id (permit_applications uses permit_id as PK)
    IF EXISTS (
      SELECT 1 FROM permit_review_comments prc
      JOIN permit_applications pa ON pa.permit_id = prc.permit_id
