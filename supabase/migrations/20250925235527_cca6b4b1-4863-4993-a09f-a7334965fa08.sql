-- Drop the broken p_is_card overload
...
-- Recreate with corrected implementation
...
BEGIN
  ...
  -- Calculate service fee using existing function
  SELECT s.service_fee_cents,
         (p_base_amount_cents + s.service_fee_cents) AS total_amount_cents
  INTO v_service_fee_cents, v_total_amount_cents
  FROM (
    SELECT public.calculate_unified_service_fee(
      p_base_amount_cents,
      COALESCE(p_is_card, false),
      p_merchant_id
    ) AS service_fee_cents
  ) s;
